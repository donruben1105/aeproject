<template>
    <div class="bg-white shadow-md rounded px-8 mt-12 pt-6 pb-8 mb-4 flex flex-col my-2">
        <div class="mb-6">
          <h1 class="text-2xl">Fill up the form to add new subjects.</h1>
        </div>
        <form @submit.prevent="submitForm">
          <div class="-mx-3 md:flex mb-6">
            <div class="md:w-1/2 px-3 mb-6 md:mb-0">
              ...
              <input v-model="form.title" class="appearance-none block w-full bg-grey-lighter text-grey-darker border border-red rounded py-3 px-4 mb-3" id="subject" type="text" placeholder="Enter subject">
            </div>
          </div>
      ...
      <div class="pt-2">    
          <div class="table w-full p-2">
              ...
              <table class="w-full border">
                  ...
                  <tbody v-else>
                      <tr v-for="subject in subjects" :key="subject.id" class="bg-gray-100 text-center border-b text-sm text-gray-600">
                          <td class="p-2 border-r">{{ subject.title }}</td>
                          <td>
                            <button @click="openUpdateModal(subject)" class="bg-blue-500 p-2 text-white hover:shadow-lg text-xs font-thin">Edit</button>
                            ...
                            <button @click="deleteSubject(subject.id)" class="bg-red-500 p-2 text-white hover:shadow-lg text-xs font-thin">Remove</button>
                          </td>
                      </tr>
                  </tbody>
              </table>
